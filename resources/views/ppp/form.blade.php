...
<form id="form-ppp" method="POST" action="{{ $isCreating ? route('ppp.store') : route('ppp.update', $ppp->id) }}">
    @csrf
    @if(!$isCreating)
    @method('PUT') {{-- interpretação Laravel, simulando métodos http que o html não suporta nativamente. Então, ao submeter o formulário, o método PUT será interpretado como um método de atualização, e não post como foi definido 3 linhas acima --}}
    @endif
